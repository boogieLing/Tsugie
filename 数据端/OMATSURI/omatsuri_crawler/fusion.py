...
from omatsuri_crawler.geocode import GeocodeClient
...
def _extract_event_year(row: dict[str, Any]) -> str:
    for field in ("event_date_start", "event_date_end", "event_name", "source_url"):
...
def _match_target_year(row: dict[str, Any], target_year: str) -> bool:
    if not target_year:
        return True
    for field in ("event_date_start", "event_date_end", "event_name", "source_url"):
        if _extract_year_token(row.get(field)) == target_year:
            return True
    return False
...
def _build_geocode_queries(row: dict[str, Any]) -> list[str]:
    queries: list[str] = []
    ...
    if venue_address:
        queries.append(venue_address)
    if prefecture or city or venue_name:
        queries.append(f"{prefecture}{city}{venue_name}")
    if prefecture and venue_name:
        queries.append(f"{prefecture}{venue_name}")
    if city and venue_name:
        queries.append(f"{city}{venue_name}")
    if venue_name:
        queries.append(venue_name)
    if prefecture and event_name:
        queries.append(f"{prefecture}{event_name}")
...
OMATSURI_INCOMPLETE_CHECK_FIELDS = [
    "expected_visitors",
    "event_time_start",
    "event_date_start",
    "venue_name",
...
def _field_incomplete_reason(field: str, value: Any) -> str:
    ...
    if any(hint in text for hint in UNCERTAIN_HINTS):
        return "uncertain"
    if field == "expected_visitors" and not re.search(r"\d", text):
...
def _compute_incomplete_tags(row: dict[str, Any], fields: list[str]) -> tuple[list[str], str]:
    ...
    if not missing_field_names:
        return tags, "none"
    if "expected_visitors" in missing_field_names or "event_time_start" in missing_field_names:
        return tags, "high"
...
def _infer_refresh_method(primary_url: str) -> str:
    ...
    if any(x in url for x in ["/event/", "/spot/", "/detail/"]):
...
def fuse_records(
    ...
    if year_filter_enabled:
        rows = [row for row in rows if _match_target_year(row, target_year_str)]
    ...
    for idx, (key, members) in enumerate(groups.items(), start=1):
        ...
        merged: dict[str, Any] = {
            ...
            "dedup_key": key,
            "event_year": next((m.get("_event_year", "") for m in members if m.get("_event_year")), ""),
            "geo_source": "",
        ...
        if lat is not None and lng is not None:
            ...
            for query in queries:
                ...
                if resp.status in {"ok", "cached_ok"} and resp.lat is not None and resp.lng is not None:
                    ...
                    if resp.status == "cached_ok":
                        merged["geo_source"] = "network_geocode_cache"
                    geocode_resolved += 1
                    resolved = True
                    break
            if not resolved:
        ...
        incomplete_tags, update_priority = _compute_incomplete_tags(merged, OMATSURI_INCOMPLETE_CHECK_FIELDS)
