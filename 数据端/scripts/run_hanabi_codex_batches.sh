...
HANABI_DIR="${ROOT_DIR}/数据端/HANABI"
CONTENT_DIR="${HANABI_DIR}/data/content"
LATEST_RUN_JSON="${HANABI_DIR}/data/latest_run.json"
ENRICH_SCRIPT="${ROOT_DIR}/数据端/scripts/enrich_event_content.py"
PYTHON_BIN_DEFAULT="${ROOT_DIR}/数据端/HANABI/.venv/bin/python"

RUN_PREFIX="${RUN_PREFIX:-20260219_hanabi_codex_polish}"
...
fused_jsonl="${HANABI_DIR}/data/fused/${fused_run_id}/events_fused.jsonl"
...
for batch in $(seq "${START_BATCH}" "${END_BATCH}"); do
  ...
  with_zh="$(jq -r '.counts.with_polished_zh // 0' "${summary_json}")"
  with_en="$(jq -r '.counts.with_polished_en // 0' "${summary_json}")"
  echo "[auto-batch] done batch=${batch} run_id=${run_id} ok=${ok} partial=${partial} empty=${empty} with_images=${with_images} with_zh=${with_zh} with_en=${with_en}" | tee -a "${master_log}"
done
