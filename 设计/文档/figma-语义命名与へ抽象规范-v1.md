...
- 适用范围：所有胶囊形态高亮（如 `chip` / `pill` / `tag` / 选中态分段控件）。
- 视觉规则：
  - 高亮态必须 `无边框`。
  - 高亮态必须使用 `渐变背景 + 多层发光`（朦胧立体感）。
  - 发光与渐变颜色必须跟随当前配色方案 token（不得写死固定颜色）。
  - 禁止使用“仅描边”或“纯色底”作为最终高亮态。
- 状态一致性：
  - 同一页面内胶囊高亮语义必须一致，不能混用不同高亮机制。
  - 主题切换后胶囊高亮需同步切换（可通过本地持久化保留上次主题选择）。
