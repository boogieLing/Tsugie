...
- 地点实体统一命名为「へ（he）」；不要在通用组件命名中硬编码“花火大会”
- 点位展示名称使用地点真实名称，不使用“xxxへ”后缀
- CTA 默认文案为“查看路线”

颜色约束：
- 参考 `schema/color-system.tsugie-he.v2.fresh.json`
- marker 颜色优先使用 gradientPairs 中的渐变色组
- 普通模式默认使用清新渐变地图背景，不使用深色底
